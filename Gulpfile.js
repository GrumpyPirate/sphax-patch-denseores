...
function resizeStream(dirname, varietyName, size) {
...
function zipStream(versionName, varietyName, packSizeName) {
    // Intended zip name:
    // [versionName] [packSizeName] Sphax Patch - patchName.zip
    // i.e.: [1.6.4] [32x] Sphax Patch - NoPatchName.zip
    // -------------------------------------------------------------------------
    var packName = packSizeName,
        zipName = '[' + versionName + '] [' + packSizeName + '] [' + varietyName + '] Sphax Patch - ' + (args.patchname || patchName) + '.zip',
        targetDir = path.join(paths.dest, versionName, packName, varietyName);
...
gulp.task('makeZips', ['optimise'], function () {
    ...
    getDirs(paths.dest, function(versionDirs) {
        // For each dir (assume these are versions)
        versionDirs.forEach(function(versionDir) {
            // And for every resize level
            getDirs(path.join(paths.dest, versionDir), function(packSizeDirs) {
                packSizeDirs.forEach(function(packSizeDir) {
                    // Get Animation variant dirs (static/animated)
                    getDirs(path.join(paths.dest, versionDir, packSizeDir), function(animVariantDirs) {
                        animVariantDirs.forEach(function(animVariantDir) {
                            // Push a new resizeStream to mergedStream
                            mergedStream.add(
                                new zipStream(versionDir, animVariantDir, packSizeDir)
                            );
                        });
                    });
                });
...
gulp.task('optimise', function () {
    ...
    getDirs(paths.src, function(versionDirs) {
        // For each dir (assume these are versions)
        versionDirs.forEach(function(versionDir) {
            // Get Animation variant dirs (static/animated)
            getDirs(path.join(paths.src, versionDir), function(animVariantDir) {
                // And for every resize level
                sizes.forEach(function(size) {
                    // Push a new resizeStream to mergedStream
                    mergedStream.add(
                        new resizeStream(versionDir, animVariantDir, size)
                    );
                }); // /sizes.forEach
            });
        }); // /dirs.forEach
